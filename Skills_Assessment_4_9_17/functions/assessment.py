...
def calculate_price(base_price, state, state_tax = .05):
    ...
    if state == "CA":
      # required a 3 percent recycle fee additional
      ca_three_percent_recycling = base_price * .03
      base_price=baseprice + ca_three_percent_recycling 
    elif state == "PA":
      highway_safety_fee=2
      base_price=base_price + highway_safety_fee
    elif state == "MA":
      if base_price < 100:
        commonwealth=1
        base_price=base_price + commonwealth_fund_fee
      if base_price > 100:
